...
class AverageMeter(object):
    ...
    def reset(self):
        self._val = [0] * self.n_items  # self.n_items=5, self._val = [0,0,0,0,0]
    ...
    def avg(self, idx=None):
        if idx is None:
            return self._sum[0] / self._count[0] if self.items is None else [self._sum[i] / self._count[i] for i in range(self.n_items)]
        else:
