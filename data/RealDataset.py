    ...
    def __init__(self, args, class_choice, split):
        ...
        if self.dataset in ['MatterPort', 'ScanNet', 'KITTI']:
            if self.dataset == 'ScanNet':
                REALDATASET = RealWorldPointsDataset(self.root+'scannet_v2_'+class_choice+'s_aligned/point_cloud', batch_size=6, npoint=2048,  shuffle=False, split=split, random_seed=0)
            elif self.dataset == 'MatterPort':
                if split in ['train', 'trainval']:
                    REALDATASET = RealWorldPointsDataset(self.root+'MatterPort_v1_'+class_choice+'_Yup_aligned/point_cloud', batch_size=6, npoint=2048,  shuffle=False, split='test', random_seed=0)
                else:
                    REALDATASET = RealWorldPointsDataset(self.root+'MatterPort_v1_'+class_choice+'_Yup_aligned/point_cloud', batch_size=6, npoint=2048,  shuffle=False, split=split, random_seed=0)
            elif self.dataset == 'KITTI':
                if split in ['train']:
                    REALDATASET = KITTIDataset(self.root+'KITTI_frustum_data_for_pcl2pcl/point_cloud_train/')
                elif split in ['test', 'val']:  
                    REALDATASET = KITTIDataset(self.root+'KITTI_frustum_data_for_pcl2pcl/point_cloud_val/')
